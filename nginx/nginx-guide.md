...
server {
    ...
    location /java-web-app-demo/ {
        ...
        sub_filter '</head>' '<script defer async client-code="YOUR-CLIENT-CODE" src="https://static.katalon.com/libs/traffic-agent/v1/traffic-agent.min.js"></script></head>';
        sub_filter_once on;
    }
...
# Expected output:
# nginx   started ~/Library/LaunchAgents/homebrew.mxcl.nginx.plist
# tomcat  started ~/Library/LaunchAgents/homebrew.mxcl.tomcat.plist
```
...
3. Look for the Katalon script in the `<head>` section:
   ```html
   <script defer async client-code="YOUR-CLIENT-CODE" src="https://static.katalon.com/libs/traffic-agent/v1/traffic-agent.min.js"></script>
   ```
